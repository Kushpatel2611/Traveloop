...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential' || err.code === 'auth/network-error') {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0B0C10] p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <p className="text-[#8B8DA3]">Sign in to continue your journey</p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-[#2a2a38] bg-[#1C1E26] accent-primary" />
                <span className="text-sm text-[#8B8DA3]">Remember me</span>
              </label>
          ...
          <div className="mt-6 text-center">
            <span className="text-[#8B8DA3]">Don't have an account? </span>
            <Link to="/register" className="text-primary font-semibold hover:text-primary-dark transition-colors">Create account</Link>
          </div>

          <div className="mt-6 pt-6 border-t border-[#1E2028]">
            <div className="flex items-center gap-4 mb-4">
              <div className="flex-1 h-px bg-[#2a2a38]"></div>
